...
const App: React.FC = () => {
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'ABOUT': return (
        <div className="flex-1 px-6 py-8 flex flex-col gap-6 animate-in fade-in overflow-y-auto scrollbar-hide">
          <h2 className="text-3xl font-normal tracking-normal dark:text-white font-display">{t.aboutTitle}</h2>
          <p className="text-base font-medium leading-relaxed text-gray-600 dark:text-gray-400 whitespace-pre-wrap">{t.aboutCopy}</p>
          <button onClick={() => setView('HOME')} className="mt-4 h-14 bg-black dark:bg-white text-white dark:text-black rounded-full border border-[#0f0f0f] dark:border-white font-normal text-base shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-transform">Back</button>
        </div>
      );
  ...
  return (
    <div className="flex items-center justify-center w-full bg-slate-50 dark:bg-black overflow-hidden" style={{height: 'var(--app-height)'}}>
      ...
      <div className="w-full h-full max-w-lg bg-white dark:bg-[#121212] flex flex-col relative overflow-hidden border-x border-gray-100 dark:border-white/5">
        ...
        <header className={`flex-shrink-0 px-5 h-16 flex justify-between items-center z-[110] bg-white dark:bg-[#121212] pt-[env(safe-area-inset-top,0px)] ${immersive ? 'hidden' : ''}`}>
           <div className="flex items-center gap-3 cursor-pointer" onClick={() => setView('HOME')}>
             ...
             <div className="flex flex-col">
               <h2 className="font-medium text-[13px] uppercase tracking-normal dark:text-white leading-none">CALMKIT</h2>
        ...
        <nav className={`flex-shrink-0 border-t border-gray-100 dark:border-white/5 bg-white dark:bg-[#121212] flex justify-around items-center h-20 pb-[env(safe-area-inset-bottom,0px)] ${immersive ? 'hidden' : ''}`}>
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 flex-1 py-1 relative active:scale-95 transition-transform ${view === navItem.id ? 'text-[#233DFF]' : 'text-gray-300'}`}
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-300 ${view === navItem.id ? 'bg-[#233DFF]/10 scale-110 shadow-sm' : ''}`}>
                {navItem.icon}
              </div>
              <span className={`text-[10px] font-medium uppercase tracking-wide ${view === navItem.id ? 'opacity-100' : 'opacity-40'}`}>
                {navItem.label}
              </span>
            </button>
          ))}
